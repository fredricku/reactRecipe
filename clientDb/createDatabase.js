...
async function createDatabase(createStatements) {
  ...
  let createUserSql = `create user if not exists '${createStatements.user}'` + `@'${createStatements.host}' identified by
  '${createStatements.password}'`;
  ...
  try {
    ...
    for (let newTable of createStatements.tables){
      ...
      let insertData = `insert into ${newTable.name} value(?)`;
